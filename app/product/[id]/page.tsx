...
export default async function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const db = getDb();
  const product = db.prepare("SELECT id, name, description, price, image, category, sizes, stock FROM products WHERE id = ? AND active = 1").get(id) as
    | { id: number; name: string; description: string; price: number; image: string; category: string; sizes: string; stock: number }
    | undefined;
